resource "azurerm_resource_group" "example234" {
  name     = var.rgname
  location = var.loc
}
...
resource "azurerm_managed_disk" "source" {
  ...
  location             = azurerm_resource_group.example234.location
  resource_group_name  = azurerm_resource_group.example234.name
